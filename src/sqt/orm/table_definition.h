...
__SQT_COLUMN_BEGIN(COLUMN_NAME, COLUMN_NAME) \
__SQT_VALUE_SOURCE_FIELD(FIELD) \
__SQT_COLUMN_DEFAULT_VALUE(DEFAULT_VALUE) \
__SQT_COLUMN_END(COLUMN_NAME, COLUMN_NAME)
...
@param INSTANCE_NAME
    The name of the index instance.

@param ...
    The names of the columns that form the index. An index can consist of up to 8 columns.

@details
    This macro is similar to `SQT_INDEX`, except that it allows specifying custom index name and 
    instance name for the index. This is useful to map the index to an existing index with the same
    columns in the database, without creating a new redundant index.
    ...
    The class name of the index type will be generated by appending the instance name to the prefix 
    `IndexType_`.
...
__SQT_DEFINE_INDEX_CUSTOM_NAME(INDEX_NAME, INSTANCE_NAME, false, __VA_ARGS__)
...
@details
    ...
    SQT_COLUMN_FIELD(ID, id)
    SQT_COLUMN_FIELD(Name, name)
    // Define an unique index with custom names and the Name column.
    SQT_INDEX_UNIQUE_2(MyEntity, Index_Name, Name)
    SQT_TABLE_END
    @endcode
...
__SQT_DEFINE_INDEX_CUSTOM_NAME(INDEX_NAME, INSTANCE_NAME, true, __VA_ARGS__)
